...
function showPanel(panelId) {
    ...
    if (target) {
        ...
        const titles = {
            'overview': 'Hub de Análise',
            'projects': 'Sistemas & Nós',
            'users': 'Banco de Dados',
            'saques': 'Gateway de Pagamentos',
            'audit': 'Auditoria Financeira'
        };
        document.getElementById('current-panel-name').innerText = titles[panelId] || 'Módulo Ativo';
    } else {
...
// ============ GRÁFICO DE CRESCIMENTO (AUDITORIA) ============
function initOrUpdateChart() {
    ...
    if (profitChart) {
        ...
        profitChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: ['6d atrás', '5d atrás', '4d atrás', '3d atrás', '2d atrás', 'Ontem', 'Hoje'],
                datasets: [{
                    label: 'Lucro (R$)',
                    data: dailyData,
                    borderColor: '#7c3aed',
...
function renderLogsTable() {
    ...
    logEntries.slice(0, 50).forEach(([id, log]) => {
        ...
        const typeLabel = log.type === 'SUSPEITA_FRAUDE' ? '⚠️ TENTATIVA FRAUDE' : '🛡️ SISTEMA';
        ...
        html += `
            <tr>
                ...
                <td><small>${log.message || 'Atividade padrão'}</small></td>
            </tr>
        `;
    });
...
function showWithdrawAlert(w) {
    ...
    if (!phone) return;
    const msg = `🚀 *NOVA SOLICITAÇÃO DE SAQUE*\n\n👤 Usuário: ${w.userName}\n💰 Valor: R$ ${parseFloat(w.amount).toFixed(2)}\n🔑 Chave PIX: ${w.pixKey}`;
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`;
    showToast(`Novo saque de R$ ${w.amount}!`, 'success');
    const notification = document.createElement('div');
    notification.innerHTML = `<div style="padding: 15px; background: #25d366; color: white; border-radius: 12px; position: fixed; bottom: 20px; right: 20px; z-index: 10000; box-shadow: 0 10px 30px rgba(0,0,0,0.3); display: flex; align-items: center; gap: 10px; cursor: pointer;" onclick="window.open('${waUrl}', '_blank')"><span>💬</span><div><strong>Novo Saque!</strong><br><small>Ver no WhatsApp</small></div></div>`;
...
function renderWithdrawalsTable() {
    ...
    pendentes.forEach(([id, w]) => {
        ...
        html += `
            <tr>
                <td>${w.userName || 'Usuário'}<br><small style="font-size:9px;">${w.uid?.substring(0,8)}</small></td>
...
async function approveWithdrawal(id) {
    if(confirm('Confirmar que o PIX já foi enviado para o usuário?')) {
...
async function cancelWithdrawal(id) {
    if(confirm('Deseja recusar este saque? O valor voltará para análise.')) {
...
function renderGlobalStats() {
    ...
    // Lê as configurações de Auditoria do banco (com valores padrão)
    const baseProfit = parseFloat(rtState.config?.baseProfit || 0.03); // Padrão dos seus $0.03
    const cpm = parseFloat(rtState.config?.cpm || 0.23); // Padrão do seu print

    // Cálculo: Lucro dos Novos Hits + O que você já tinha
    const newProfitUSD = (cineHits / 1000) * cpm;
    const profitUSD = baseProfit + newProfitUSD;
    const profitBRL = profitUSD * 5.15; // Câmbio atualizado
    ...
    // Preenche os inputs no menu Auditoria para você poder alterar depois
...
// ============ GESTÃO DE USUÁRIOS (TABELA COM ANTI-FRAUDE) ============
function renderUsersTable(filter = '') {
    ...
    usersEntries.forEach(([uid, user]) => {
        ...
        const deviceId = user.deviceId || uid; // Fallback para UID se deviceId não existir
        const isBanned = rtState.devices[deviceId]?.banned === true;
        const securityStatus = isBanned
            ? '<span style="color: #ff4d4d; font-weight: 800;">🚫 BANIDO</span>'
            : '<span style="color: #00ff95; font-weight: 800;">🛡️ LIMPO</span>';
...
async function toggleDeviceBan(deviceId, currentlyBanned) {
    ...
    if (confirm(`Deseja ${action} este hardware? O usuário não conseguirá acessar nem com outras contas.`)) {
        try {
...
async function editUserBalance(uid, currentBalance) {
    const newBalance = prompt(`Novo saldo para o usuário ${uid}:`, currentBalance);
    if (newBalance !== null) {
...
async function deleteUser(uid) {
    if (confirm(`Excluir usuário ${uid} do banco de dados?`)) {
...
function updateSystemConfig(sysId, key, value) {
    hubDb.ref(`config/${key}`).set(value)
        .then(() => showToast('Configuração salva!', 'success'));
}
...
function updateSystemInterface() {
    ...
    // Segurança de ID Único: Ativa por padrão se não houver valor no banco
    ...
    if (badge) {
        if (status === false) {
            ...
            badge.innerText = '🛠️ MANUTENÇÃO';
            badge.className = 'sys-badge maintenance';
        } else {
            badge.innerText = '✅ NODE ATIVO';
...
// ============ CONTROLES DE INTERFACE RETRÁTIL ============
